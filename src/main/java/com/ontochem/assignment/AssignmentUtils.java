...
public class AssignmentUtils {
  ...
  hierarchicalParallelClassAssignment( String rootClassId, String _module, int _nThreads, 
                                       List<String> _ocidSetIn, 
                                       Map<String,String> _ocid2smilesMap, 
                                       Map<String,List<String>> _ocidClass2smartsList, 
                                       Map<String,Set<String>> _ocidClass2childMap ) 
  throws IOException {
					...
					while ( ! classIdList.isEmpty() ) {
						...
						for ( String classId : classIdList ) {
						  ...
						  if ( assignedList.contains( classId ) ) continue;
							...
							if ( ( smartsList != null ) ) {
								if ( assign( smiles, smartsList, _module ) ) {
   ...
   */
	public static boolean assign( String _smiles, List<String> _smartsList, String _module ) {
		...
		if ( ChemLib.CHEMLIB_CA.equals( _module ) ) {
		  ...
		  return assignCdkOrAmbit( _smiles, _smartsList, _module);
  ...
  // ------------------------------------------------------------------------
	 ...
	 * @return
	 */
	public static boolean assignChemaxon( String _smiles, List<String> smartsList, String module ) {
		try {
			...
			for ( int i=0; i < smartsList.size(); i++ ) {
	    		...
	    		if ( Qry2.equals(Qry) ) {
	    			...
	    			if( Qry1.equals( Qry ) ) {
	    				 ...
	    				 */
		    			int rspd = SubStructureSearchEngine( _smiles, Qry, module );
	    				...
	    				for ( int j = 0; j < newQryList.size(); j++ ) {
	    					...
	    					int rspd = SubStructureSearchEngine( _smiles, newQry, module);
	    			...
	    			if( Qry1.equals( Qry2 ) ) {
	    				 ...
	    				 */
	    				int rspd1 = SubStructureSearchEngine( _smiles, Qry2, module);
	 ...
	 * @throws Exception
	 */
	public static int checkQueryMultiplicity( String smi, String sma, String module ) throws Exception {
		try  {
			...
			if ( exact.equals( sma ) ) {
				...
				if ( more.equals( sma ) ) {
					//does not contain MORE keyword
	 				return StructureSearchEngine.searchBySubstructure( smi, sma, module );
				} else {
	 ...
	 */
	public static boolean assignCdkOrAmbit( String _smiles, List<String> smartsList, String module ) {
		try {
			List<String> ORResponse = new ArrayList<String>();
			List<String> NOTResponse = new ArrayList<String>();
	    		...
	    		if ( Qry2.equals(Qry) ) {
	    			...
	    			if( Qry1.equals( Qry ) ) {
	    				...
	    				if( Qry3.equals( Qry ) ) {
		    				...
		    				//no patterns found, OR structure
			    			int rspd = checkQueryMultiplicity( _smiles, Qry, module );
			    			if ( rspd >0 ) ORResponse.add( "true" );
		    				else ORResponse.add("false");
		    				isORstructure = true;
			    			//System.out.println("sss: " + " OR " + rspd + " " + Qry);
	    			...
	    			if( Qry1.equals( Qry2 ) ) {
	    				...
	    				if( Qry3.equals( Qry2 ) ) {
		    				...
		    				// no patterns found, OR structure preceeded by NOT
		    				int rspd1 = checkQueryMultiplicity( _smiles, Qry2, module);
		    				if ( rspd1 >0 ) NOTResponse.add("false");
	    					else NOTResponse.add("true");
			    			isNOTstructure = true;
	 ...
	 */
	private static int SubStructureSearchEngine( String target, String query, String module ) {
		try {
			return StructureSearchEngine.searchBySubstructure( target, query, module ) ;
		} catch (Exception e)  {
